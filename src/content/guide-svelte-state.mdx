...
<InfoQuote>
  ℹ️ **Note:** Use [`derived.by(fn)`](https://svelte.dev/docs/svelte/$derived#$derived.by) for more
  complex logic instead of the shorthand `derived(expression)`.
</InfoQuote>
...
<InfoQuote>
  ℹ️ **Note:** svelte optimizes away redundant assignments such as `upvotes.count = upvotes.count`
  or `upvotes.count += 0` will invalidate state updates.
</InfoQuote>
